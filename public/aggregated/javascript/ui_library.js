...
/* ----------------------------------------------------------------------
   - app/views/shared/confirm_lightbox.coffee (last modified: 2016-01-21 00:14:18 +0000)
   ---------------------------------------------------------------------- */
(function() {
  var bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
  ...
  Scribd.ConfirmLightbox = (function(superClass) {
    extend(ConfirmLightbox, superClass);

    function ConfirmLightbox(container) {
      this.set_choose_callback = bind(this.set_choose_callback, this);
      this.set_confirm_callback = bind(this.set_confirm_callback, this);
      this.set_template = bind(this.set_template, this);
      this.close = bind(this.close, this);
      ConfirmLightbox.__super__.constructor.apply(this, arguments);
      this.templates = this.container.find(".confirmation_lightbox_templates");
      this.container.on('click', '.ok_btn', (function(_this) {
        return function(e) {
          _this.container.trigger('scribd:confirmation:ok', e.target);
          if (typeof _this.confirm_callback === "function") {
            _this.confirm_callback();
          }
          if (typeof _this.choose_callback === "function") {
            _this.choose_callback();
          }
          return _this.close();
        };
      })(this));
      this.container.on('click', '.cancel_btn', (function(_this) {
        return function(e) {
          _this.container.trigger('scribd:confirmation:cancel', e.target);
          if (typeof _this.choose_callback === "function") {
            _this.choose_callback();
          }
          return _this.close();
    ...
    ConfirmLightbox.prototype.close = function() {
      if (typeof this.choose_callback === "function") {
        this.choose_callback();
      }
      return ConfirmLightbox.__super__.close.apply(this, arguments);
    };

    ConfirmLightbox.prototype.set_template = function(name, opts) {
      var template, tmpl;
      opts = _.extend({
        simple_style: false
      }, opts);
      template = this.templates.data(name);
      if (!template) {
        throw new Error("Template " + name + " not found!");
      }
      if (opts.title) {
        this.container.find(".title").text(opts.title);
      }
      this.container.toggleClass("simple_style vertically_centered", opts.simple_style);
      tmpl = _.template(template, opts);
      this.container.find(".rendered_template_container").html(tmpl);
      return this.set_position();
    };

    ConfirmLightbox.prototype.set_confirm_callback = function(fn) {
      this.confirm_callback = fn;
      return this;
    };

    ConfirmLightbox.prototype.set_choose_callback = function(fn) {
      this.choose_callback = fn;
      return this;
    };
...
/* ----------------------------------------------------------------------
   - app/views/ui_library/index.coffee (last modified: 2016-01-15 19:39:42 +0000)
   ---------------------------------------------------------------------- */
(function() {
  Scribd.UI.Library = (function() {
    function Library(container) {
      this.container = container;
      this.container = $(this.container);
      this.flasher = new Scribd.Flasher();
      this.setup_copy();
      this.container.on("click", ".documentation_collapse", (function(_this) {
        return function(e) {
          var doc, el;
          el = $(e.target);
          doc = _this.container.find(".documentation[data-name='" + (el.data('name')) + "']");
          if (doc.is(":visible")) {
            return doc.slideUp();
          } else {
            return doc.slideDown();
          }
        };
      })(this));
      this.container.find(".floating_nav").first().stick_in_parent({
      ...
      $(window).on("scroll", (function(_this) {
        return function() {
          _this.update_scrollspy();
          return $(document).trigger("sticky_kit:recalc");
        };
      })(this));
    ...
    Library.prototype.setup_copy = function() {
      var copy_selectors;
      if (this.is_copy_setup) {
        return;
      }
      copy_selectors = [".copy_btn", ".variable"].join(',');
      this.container.find(copy_selectors).attr("title", "Click to copy");
      return this.is_copy_setup = typeof ZeroClipboard !== "undefined" && ZeroClipboard !== null ? (ZeroClipboard.config({
        swfPath: "/swf/ZeroClipboard.swf"
      }), this.zero_clip = new ZeroClipboard(this.container.find(copy_selectors)), this.zero_clip.on("aftercopy", (function(_this) {
        return function(e) {
          return _this.flasher.show('info', e.data['text/plain'] + " copied to clipboard");
        };
      })(this)), true) : void 0;
    };

    Library.prototype.setup_scrollspy = function() {
      var i, len, link, ref;
      if (this.is_scrollspy_setup) {
        return;
      }
      this.spy_elements = [];
      ref = this.container.find(".section_link_container");
      for (i = 0, len = ref.length; i < len; i++) {
        link = ref[i];
        this.spy_elements.push($(link).children("a"));
      }
      this.update_scrollspy();
      return this.is_scrollspy_setup = true;
    };

    Library.prototype.update_scrollspy = function() {
      var elm, i, len, ref, results, top, window_offset;
      if (!this.is_scrollspy_setup) {
        return;
      }
      this.scrollspy_active_idx || (this.scrollspy_active_idx = 0);
      window_offset = $(window).scrollTop();
      ref = this.spy_elements;
      results = [];
      for (i = 0, len = ref.length; i < len; i++) {
        elm = ref[i];
        top = $(elm.attr("href")).offset().top - window_offset;
        if (top > 0 && top < 400) {
          this.container.find(".section_link_container.selected").removeClass("selected");
          elm.closest(".section_link_container").addClass("selected");
          break;
        } else {
          results.push(void 0);
        }
